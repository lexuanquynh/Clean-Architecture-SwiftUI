...
public struct HomeFeedView: View {
                ...
                if viewModel.posts.isEmpty && !viewModel.isLoading {
                    ...
                    ScrollView {
                        LazyVStack(spacing: 16) {
                            ...
                            // Load More Indicator
                            if viewModel.hasMorePosts && viewModel.searchText.isEmpty {
                                ProgressView()
                                    .padding()
                                    .onAppear {
                                        Task {
    ...
    set: { newValue in
        if !newValue { viewModel.selectedPost = nil }
    }
)) {
                    ...
                    if !viewModel.relatedPosts.isEmpty {
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            ScrollView(.horizontal, showsIndicators: false) {
                                HStack(spacing: 16) {
                                    ForEach(viewModel.relatedPosts.indices, id: \.self) { idx in
                                        let related = viewModel.relatedPosts[idx]
                                        RelatedPostCard(post: related)
                                    }
                                }
                                .padding(.horizontal)
                            }
